...
class TimetableService {
  ...
  static async parseExcelCourse(excelRow, transaction) {
    try {
      ...
      if (!courseCode || !courseName || !credits || isNaN(credits) || courseCode.includes("총건수")) {
        return null;
      }
  ...
  static assignSemesterColors(slots) {
    const colors = ["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#ffde74ff", "#DDA0DD", "#98D8E8", "#ffb65eff", "#BB8FCE", "#85C1E9"];
  ...
  static async create(userId, { semesterCode, year, courses }) {
    ...
    try {
      const schedule = await Schedule.create({ userId, semesterCode, year: year || new Date().getFullYear() }, { transaction });
  ...
  static async generateTimetables(userId, curriculumId, preferences = {}, excludeGenerated = []) {
    try {
      ...
      const scoredTimetables = this.scoreAndRankTimetables(mergedTimetables, normalizedPreferences);
      const topTimetables = scoredTimetables.slice(0, 5);
      const isAvoidFailed = topTimetables.some((table) => table.is_avoid_failed === "Y") ? "Y" : "N";
      const avoidFailedReasons = Array.from(
        new Set(topTimetables.flatMap((table) => table.avoid_failed_reasons || []))
      );
  ...
  static async findMatchingLectures(curriculumLectures) {
    ...
    for (const curriLecture of curriculumLectures) {
      ...
      if (notFoundLectures.length > 0) {
        ...
        notFoundLectures.forEach((l) => console.log(`- ${l.name} (lect_id=${l.lect_id}, ${l.grade}학년 ${l.semester}학기, ${l.credits}학점)`));
      }
    }
  ...
  // 유효한 시간표 조합 생성
  static async generateValidCombinations(lectureGroups, preferences, excludeGenerated = [], curriculumLectures = []) {
  ...
  static canAddLecture(currentLectures, newLecture, preferences, lectureGroups) {
    ...
    if (preferences.avoid_professors?.includes(newLecture.professor)) {
      const alternatives = lectureGroups[newLecture.name]?.filter((l) => l.professor !== newLecture.professor);
    ...
    if (!this.checkTimePreferences(newLecture, preferences)) {
      const alternatives = lectureGroups[newLecture.name]?.filter((l) => this.checkTimePreferences(l, preferences));
      if (alternatives && alternatives.length > 0) {
        return false;
      }
    }
  ...
  static isValidTimetable(lectures, preferences, curriculumLectures = []) {
    ...
    const requiredCredits = curriculumLectures.filter((curri) => matchedLectureIds.has(curri.lect_id)).reduce((sum, curri) => sum + curri.credits, 0);
  ...
  static scoreAndRankTimetables(timetables, preferences) {
    const scoredTimetables = timetables.map((timetable) => {
      ...
      const { failed: avoidFailed, reasons: avoidFailedReasons } = this.evaluateAvoidPreferenceFailure(
        timetable.lectures,
        preferences
      );
      ...
      return {
        id: timetable.id,
        score,
        is_avoid_failed: avoidFailed ? "Y" : "N",
        avoid_failed_reasons: avoidFailedReasons,
  ...
  static calculateTimetableScore(lectures, preferences) {
    ...
    if (preferences.free_days) {
      ...
      const freeDayBonus = preferences.free_days.filter((day) => !occupiedDaysSet.has(day.toLowerCase())).length * 7;
      score += freeDayBonus;
    }
  ...
  static async saveTimetable(userId, timetable, semesterCode, { isGenerated = false } = {}) {
    ...
    try {
      ...
      for (const lecture of lectures) {
        ...
        const schedules = isGenerated ? lecture.schedule : this.mergeConsecutiveSchedule(lecture.schedule);
        ...
        for (const slot of schedules) {
          ...
          const [startTime, endTime] = slot.start_end
            ...
            : [this.periodToTime(startPeriod).start, this.periodToTime(endPeriod).end];
  ...
  // 헬퍼 메서드들
  static normalizePreferenceList(value) {
    if (!value) return [];
  ...
  static normalizePreferences(preferences = {}) {
    ...
    normalized.free_days = this.normalizePreferenceList(preferences.free_days).map((day) => day.toLowerCase());
    normalized.preferred_professors = this.normalizePreferenceList(preferences.preferred_professors);
    normalized.avoid_professors = this.normalizePreferenceList(preferences.avoid_professors);

    if (normalized.max_consecutive_hours !== undefined && normalized.max_consecutive_hours !== null) {
      const parsed = parseInt(normalized.max_consecutive_hours, 10);
      normalized.max_consecutive_hours = Number.isNaN(parsed) ? undefined : parsed;
    }

    if (normalized.max_daily_classes !== undefined && normalized.max_daily_classes !== null) {
      const parsed = parseInt(normalized.max_daily_classes, 10);
      normalized.max_daily_classes = Number.isNaN(parsed) ? undefined : parsed;
    }

    if (typeof normalized.exclude_morning === "string") {
      normalized.exclude_morning = normalized.exclude_morning === "true";
    }
    if (typeof normalized.exclude_evening === "string") {
      normalized.exclude_evening = normalized.exclude_evening === "true";
    }

    return normalized;
  }

  static evaluateAvoidPreferenceFailure(lectures, preferences = {}) {
    if (!lectures || lectures.length === 0) {
      return { failed: false, reasons: [] };
    }

    const avoidProfessors = new Set(
      this.normalizePreferenceList(preferences.avoid_professors).map((name) => name.toLowerCase())
    );
    const freeDays = new Set((preferences.free_days || []).map((day) => day.toLowerCase()));
    const maxDailyClasses = preferences.max_daily_classes;
    const maxConsecutiveHours = preferences.max_consecutive_hours;
    const reasons = [];

    if (avoidProfessors.size > 0) {
      const hasAvoidProfessor = lectures.some((lecture) => {
        const professorNames = (lecture.professor || "")
          .split("/")
          .map((name) => name.trim().toLowerCase())
          .filter(Boolean);
        return professorNames.some((name) => avoidProfessors.has(name));
      });
      if (hasAvoidProfessor) {
        reasons.push("avoid_professors");
      }
    }

    if (preferences.exclude_morning) {
      const hasMorning = lectures.some((lecture) =>
        (lecture.schedule || []).some((slot) => {
          const startPeriod = slot.startPeriod ?? slot.period;
          return typeof startPeriod === "number" && startPeriod <= 1;
        })
      );
      if (hasMorning) {
        reasons.push("exclude_morning");
      }
    }

    if (preferences.exclude_evening) {
      const hasEvening = lectures.some((lecture) =>
        (lecture.schedule || []).some((slot) => {
          const endPeriod = slot.endPeriod ?? slot.period;
          return typeof endPeriod === "number" && endPeriod >= 12;
        })
      );
      if (hasEvening) {
        reasons.push("exclude_evening");
      }
    }
    ...
    if (freeDays.size > 0) {
      ...
      lectures.forEach((lecture) => {
        (lecture.schedule || []).forEach((slot) => {
          if (slot.day) {
            occupiedDays.add(slot.day.toLowerCase());
          }
        });
      });
      ...
      for (const day of freeDays) {
        if (occupiedDays.has(day)) {
          reasons.push("free_days");
          break;
        }
      }
    }

    if (typeof maxDailyClasses === "number" && maxDailyClasses > 0) {
      const dailyCounts = {};
      lectures.forEach((lecture) => {
        const daysForLecture = new Set();
        (lecture.schedule || []).forEach((slot) => {
          if (slot.day) {
            daysForLecture.add(slot.day.toLowerCase());
          }
        });
        daysForLecture.forEach((day) => {
          dailyCounts[day] = (dailyCounts[day] || 0) + 1;
        });
      });

      if (Object.values(dailyCounts).some((count) => count > maxDailyClasses)) {
        reasons.push("max_daily_classes");
      }
    }

    if (typeof maxConsecutiveHours === "number" && maxConsecutiveHours > 0) {
      const dayPeriods = {};
      lectures.forEach((lecture) => {
        (lecture.schedule || []).forEach((slot) => {
          if (!slot.day) return;
      ...
      for (const periods of Object.values(dayPeriods)) {
        ...
        for (let i = 1; i < uniquePeriods.length; i += 1) {
          if (uniquePeriods[i] === uniquePeriods[i - 1] + 1) {
            current += 1;
          } else {
        ...
        longest = Math.max(longest, current);
        if (longest > maxConsecutiveHours) {
          reasons.push("max_consecutive_hours");
          break;
        }
      }
    }
  ...
  static assignUniqueColor(courseName, courseColorMap, usedColors) {
    const palette = ["#FF6B6B", "#4ECDC4", "#45B7D1", "#96CEB4", "#FFEAA7", "#DDA0DD", "#98D8E8", "#F7DC6F", "#BB8FCE", "#85C1E9"];
